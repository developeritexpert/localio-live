...
class BusinessForm extends Component
{
    ...
    //     // Fetch the business data if available
    //     if ($businessId) {
    //         $business = Business::with(['countries', 'websites', 'pricingOptions', 'features'])->findOrFail($businessId);
    ...
    //         // Populate selected countries (based on business relationships)
    //         $this->selectedCountries = $business->countries->pluck('id')->toArray();

    //         // Populate country-specific URLs (assuming a relationship exists)
    ...
    //     // Fetch the list of all available countries
    //     $this->countries = Country::all();

    //     // Load category features if a category is selected
    ...
    public function mount($businessId = null, $faqBusinessId = null, $faqEditId = null, $pageMode = null)
    {
        ...
        // 🧠 Your original business mount logic continues here:
        $this->selectedPricingOptions = [];
        $this->selectedFeatures = [];
        $this->ai_reference_url = '';
        $this->loadInitialData();
    ...
    protected function loadCategoryFeatures($categoryId)
    {
        ...
        $this->categoryFeatures = Feature::where('category_id', $categoryId)
            ->where('status', 'active')
    ...
    public function render()
    {
        return view('livewire.business-form', [
            'countries' => $this->countries, // Pass the countries data to the Blade view
        ]);
    }
    ...
    public function BusinessImageSave()
    {
        ...
        foreach ($this->new_business_images as $image) {
            if (is_object($image)) {
                $fileName = $this->storeFile($image, 'business_gallery');
