        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">Vendor Information</h3>
                </div>
            </div>
        </div>
        <div class="nk-block">
            <div class="card card-bordered card-stretch">
                <div class="card-inner-group">
                    <div class="card-inner p-2">
                        <form action="{{ route('admin-update-vendors') }}" id="user-form" method="post">
                            @csrf
                            <input type="hidden" id="id" name="id" value="{{ $user->id ?? ''}}">
                            <input type="hidden" id="role" name="role" value="{{ $user->user_type ?? 'vendor'}}">
                            ...
                            {{-- <div class="col-lg-12 d-flex">
                                <div class="col-lg-12 p-3 form-group mb-3">
                                    ...
                                    <div class="form-control-wrap">
                                        <select class="form-select form-control-lg" name="role" id="role">
                                            <option value="user" {{ isset($user->user_type) && $user->user_type == 'user' ? 'selected' : '' }}>User</option>
                                            <option value="admin" {{ isset($user->user_type) && $user->user_type == 'admin' ? 'selected' : '' }}>Admin</option>
                                            <option value="vendor" {{ isset($user->user_type) && $user->user_type == 'vendor' ? 'selected' : '' }}>Vendor</option>
