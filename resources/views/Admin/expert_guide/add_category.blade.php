    ...
    <div class="card card-bordered">
        <div class="card-inner">
            <div class="nk-block">
                @if(isset($category))
                    @php
                        $translation = isset($category) ? $category->expertGuideCategoryTranslation->first() : null;
                    @endphp
                @endif
                {{-- <pre>{{ dd($category->id) }}</pre> --}}

                <form action="{{ route('admin-expert-guide-store-category') }}" class="form-validate"
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <input type="text" class="form-control" name="name" id="name" value="{{ old('name', $translation->name ?? '') }}">
                                </div>
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <textarea class="description" name="description" id="description" rows="4"
                                        cols="50">{{ old('description', $translation->description ?? '') }}</textarea>
                                </div>
                                @error('description')
                                <div class="error text-danger">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                @if(isset($category->image))
                                    <img src="{{ asset( $category->image) }}" alt="{{ $category->name }}" style="width: 50px; height: auto;">
                                @else
                                        <span>No Image</span>
                                @endif
                                @endif
