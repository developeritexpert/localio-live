...
<div class="nk-block nk-block-lg">
    <div class="nk-block-head d-flex justify-content-between align-items-center">
        <div class="nk-block-head-content">
            <h4 class="title nk-block-title">Edit Category Feature Content</h4>
            <p class="text-muted">Update custom top description and dynamic text sections (H2 & H3).</p>
        </div>
    ...
    <div class="card card-bordered">
        <div class="card-inner">
            <form action="{{ route('admin.category-feature-content.update', $content->id) }}" method="POST" class="form-validate">
                @csrf
                <div class="row g-4">
                    <!-- Category Selection -->
                    <div class="col-md-5">
                            ...
                            <div class="form-control-wrap">
                                <select name="category_id" id="category_id" class="form-select js-select2" required>
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ (old('category_id', $content->category_id) == $cat->id) ? 'selected' : '' }}>
                                            {{ $cat->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-control-wrap">
                                <select name="feature_id" id="feature_id" class="form-select js-select2" required>
                                    @foreach($features as $feat)
                                        <option value="{{ $feat->id }}" {{ (old('feature_id', $content->feature_id) == $feat->id) ? 'selected' : '' }}>
                                            {{ $feat->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-control-wrap">
                                <select name="lang_id" id="lang_id" class="form-select" required>
                                    @foreach($languages as $lang)
                                        <option value="{{ $lang->id }}" {{ (old('lang_id', $content->lang_id) == $lang->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <div class="form-control-wrap">
                                <textarea name="description" id="description" class="form-control rich-editor" rows="5" placeholder="Enter introductory description for this category-feature page...">{{ old('description', $content->description) }}</textarea>
                            </div>
                                        ...
                                        <div class="section-card border rounded p-3 mb-3 bg-white" data-section-index="{{ $sIndex }}">
                                            ...
                                            <div class="row g-3">
                                                <div class="col-12">
                                                    ...
                                                    <input type="text" class="form-control" name="text_sections[{{ $sIndex }}][h2_title]" value="{{ $sec['h2_title'] ?? '' }}" placeholder="e.g. Why choose shared hosting with noise cancellation?">
                                                </div>
                                                <div class="col-12">
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <div class="sub-sections-container ps-3 border-start" id="subSectionsContainer_{{ $sIndex }}">
                                                        @if(isset($sec['sub_sections']) && is_array($sec['sub_sections']))
                                                            ...
                                                            <div class="sub-section-item border rounded p-2 mb-2 bg-light" data-sub-index="{{ $subIndex }}">
                                                                ...
                                                                <input type="text" class="form-control form-control-sm mb-2" name="text_sections[{{ $sIndex }}][sub_sections][{{ $subIndex }}][h3_title]" value="{{ $sub['h3_title'] ?? '' }}" placeholder="e.g. Key Features & Highlights">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let sectionCounter = {{ isset($text_sections) && is_array($text_sections) ? count($text_sections) : 0 }};
